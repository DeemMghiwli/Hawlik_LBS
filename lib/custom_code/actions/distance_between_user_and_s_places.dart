...
int? distanceBetweenUserAndSPlaces(LatLng? positionTwo) {
  ...
  userRef.doc(currentUser.uid).get().then((userDoc) {
    ...
    shopRef.get().then((shopSnapshot) {
      ...
      shopSnapshot.docs.forEach((shopDoc) {
        print("health Document Data: ${shopDoc.data()}");
        final foodL = shopDoc['location'];
        if (shopDoc.data().containsKey('location') &&
